...
def load_blip():
    ...
    Model : Salesforce/blip-image-captioning-base (pre-trained)
    Runs  : LOCAL with float16 to save RAM (~250MB vs ~500MB full precision)
    Fix   : Previously used HF Inference API which does not support BLIP
            → now loaded locally to avoid BadRequestError
    """
